...
def mean_cer(ground_truth: dict, predictions: dict) -> float:
...
def mean_wer(ground_truth: dict, predictions: dict) -> float:
    matches = _match_gt_results(ground_truth, predictions)
...
def mean_teds(ground_truth: dict, predictions: dict) -> float:
    matches = _match_gt_results(ground_truth, predictions)

    total_teds = sum(
        teds(match['ground_truth'], match['prediction'])
        for match in matches
    )
...
def mean_oss(ground_truth: dict, predictions: dict) -> float:
    matches = _match_gt_results(ground_truth, predictions)

    total_oss = sum(
        ordered_sequence_similarity(match['ground_truth'], match['prediction'])
        for match in matches
    )
